@tool
extends Path2D

enum PathMode { LINEAR, CIRCLE }

@export_group("Mode Control")
@export var mode: PathMode = PathMode.LINEAR:
	set(value):
		mode = value
		if not (mode == PathMode.LINEAR and lock_linear_path):
			_update_path()

@export var lock_linear_path: bool = false

@export_group("Timing")
@export var travel_time: float = 10.0
@export var one_way: bool = true

@export_group("Circle Settings")
@export var radius: float = 100.0:
	set(value):
		radius = value
		if mode == PathMode.CIRCLE: _update_path()

@export_range(4, 64) var points_count: int = 4:
	set(value):
		points_count = value
		if mode == PathMode.CIRCLE: _update_path()

var direction: float = 1.0
var flip_flip: bool = true # if we go from right to left, flip orientation

func _ready():
	if not Engine.is_editor_hint():
		var follower = get_node_or_null("PathFollow2D")
		if follower:
			follower.loop = (mode == PathMode.CIRCLE)
		var shark = get_node_or_null("PathFollow2D/Shark")
		if shark:
			var x1 = curve.get_point_position(0).x
			var x2 = curve.get_point_position(1).x
			flip_flip = x1 < x2
		_flip_shark(false)
		
func _process(delta: float):
	if Engine.is_editor_hint(): return
	
	var follower = get_node_or_null("PathFollow2D")
	if not follower or not curve or travel_time <= 0: return

	var total_length = curve.get_baked_length()
	if total_length <= 0: return
	
	var speed = total_length / travel_time
	
	if mode == PathMode.CIRCLE:
		follower.loop = true
		follower.progress += speed * delta
		
		if follower.progress_ratio > 0.01 and follower.progress_ratio < 0.49:
			_flip_shark(false) # Facing right
		else:
			_flip_shark(true) # Facing left
			
	else:
		follower.loop = false
		follower.progress += speed * delta * direction * (1 if one_way else 2)
		
		if direction > 0 and follower.progress_ratio >= 0.999:
			direction = -1.0
			_flip_shark(true)
		elif direction < 0 and follower.progress_ratio <= 0.001:
			direction = 1.0
			_flip_shark(false)

func _flip_shark(flip_to: bool):
	var follower = get_node_or_null("PathFollow2D/Shark")
	if follower: follower.flip_h(flip_to ==flip_flip)

func _update_path():
	if mode == PathMode.LINEAR and lock_linear_path: 
		return
	
	curve = Curve2D.new()
	
	if mode == PathMode.LINEAR:
		curve.add_point(Vector2.ZERO)
		curve.add_point(Vector2(300, 0))
	else:
		_generate_circle_points()
	
	queue_redraw()

func _generate_circle_points():
	var control_length = radius * (4.0 / 3.0) * tan(PI / (2.0 * points_count))
	
	for i in range(points_count + 1):
		var angle = (i * TAU / points_count) + PI
		var pos = Vector2(cos(angle), sin(angle)) * radius
		var dir = Vector2(-sin(angle), cos(angle))
		curve.add_point(pos, -dir * control_length, dir * control_length)
